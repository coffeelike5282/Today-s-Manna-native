...
echo [2/3] 한국 표준시(KST) 및 한글(ko-KR) 설정 중...
%ADB% -s emulator-5554 shell settings put global auto_time 0
%ADB% -s emulator-5554 shell settings put global auto_time_zone 0
%ADB% -s emulator-5554 shell settings put global timezone Asia/Seoul
%ADB% -s emulator-5554 shell settings put system system_locales ko-KR
%ADB% -s emulator-5554 shell settings put global system_locales ko-KR

echo [3/3] 설정 완료! 현재 에뮬레이터 시간 확인:
%ADB% -s emulator-5554 shell date
echo.

echo 박 사장님, 시간대 및 언어 설정 완료했습니다! 시원하시죠? 충성! 🫡
